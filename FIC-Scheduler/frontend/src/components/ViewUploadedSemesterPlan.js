...
export function ViewUploadedSemesterPlan() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [allSemesterPlansResponse, courseOfferedResponse, classroomAvailableResponse, instructorsResponse] =
                    await Promise.all([
                ...
                const courseOfferedById = courseOfferedData.reduce((acc, courseOffering) => {
                ...
                const classroomAvailableById = classroomAvailableData.reduce((acc, classroom) => {
                        ...
                        <TableBody>
                            {allSemesterPlans.map((row, rowIndex) => (
                                <TableRow key={rowIndex}>
                                    <TableCell key="0" sx={{ verticalAlign: "top", padding: "10px" }}>{row.id}</TableCell>
                                    <TableCell key="1" sx={{ verticalAlign: "top", padding: "10px" }}>{row.name}</TableCell>
                                    <TableCell key="2" sx={{ verticalAlign: "top", padding: "10px" }}>{row.notes}</TableCell>
                                    <TableCell key="3" sx={{ verticalAlign: "top", padding: "10px" }}>{row.semester}</TableCell>
                                    <TableCell key="4" sx={{ verticalAlign: "top", padding: "10px" }}>
                                    ...
                                    <TableCell key={`instructors-${rowIndex}`} sx={{ verticalAlign: "top", padding: "10px" }}>
                                        {renderInstructorAvailability(row.instructorsAvailable)}
                                    </TableCell>
                                    <TableCell key="6" sx={{ verticalAlign: "top", padding: "10px" }}>
