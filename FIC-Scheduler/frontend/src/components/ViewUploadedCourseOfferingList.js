...
export function ViewUploadedCourseOfferingList() {
    ...
    useEffect(() => {
        // Fetch both course offerings and instructors
        const fetchData = async () => {
            try {
                const [courseOfferingsResponse, instructorsResponse] = await Promise.all([
                ...
                console.log("Instructors Map:", instructorsById);
                        ...
                        <TableBody>
                            {allCourseOfferings.map((row, rowIndex) => (
                                <TableRow key={rowIndex}>
                                    ...
                                    <TableCell key="4">
                                        {row.approvedInstructors.map((instructorId) => (
